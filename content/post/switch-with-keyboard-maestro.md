...
对于切换系统默认浏览器来说，也是一个类似的操作。一般情况下，我们只能在 [系统设置][8] 中进行修改。
...
与切换输入法不同的是，一般还可以直接在浏览器内的设置中将其设置为系统默认浏览器。
...
{{< imgcap title="用 Keyboard Maestro 切换英文 ABC 和中文双拼输入法" src="https://p15.p3.n0.cdn.getcloudapp.com/items/WnuZplj4/eb8965fd-e47d-4be7-a91e-aeeecba667a5.png" >}}

由于切换输入法是一个相当高频的操作，为了找回我熟悉的快捷切换方式，我没有使用快捷键触发，而选择的触发条件是「USB Device Key Trigger」，因为这样可以实现「**短按左 ⌘ Command 键，切换为中文双拼输入法，短按左 ⌥ Option 键，切换为英文 ABC 输入法**」，如下图所示。

{{< imgcap title="短按左 ⌘ Command 键，切换为中文双拼输入法，短按左 ⌥ Option 键，切换为英文 ABC 输入法" src="https://p15.p3.n0.cdn.getcloudapp.com/items/xQuONY1A/d6cbd172-1087-4400-bc60-01505e9f76b6.gif" >}}
...
尽管 Keyboard Maestro 无法将「[光标定位到浏览器地址][17]」作为触发条件，但我们可以用快捷键山寨一个类似的操作。在 Safari、Chrome、Firefox 等主流浏览器中，定位到地址栏的快捷键是 ⌘ Command + L，它的名称一般叫作「Open Location…」，因此可以将触发条件设置为快捷键 Command + L，然后将输入法切换为 ABC，再模拟按下快捷键 Command + L，如下图所示。
...
{{< imgcap title="在浏览器地址栏切换输入法为 ABC" src="https://p15.p3.n0.cdn.getcloudapp.com/items/qGuZYXOD/bcf1b447-3f94-45d5-8693-44d89be0eff7.gif" >}}
...
{{< imgcap title="Keyboard Maestro 论坛中的一个切换浏览器的复杂动作" src="https://p15.p3.n0.cdn.getcloudapp.com/items/geuO1oLP/d797b461-c1b4-4d49-a175-1e319fe7c1d4.png" >}}
...
2. 使用 AppleScript 在弹出式窗口中进行确认

为了方便快捷地实现这个操作，我在 Keyboard Maestro 中创建了一个切换浏览器的组，其中有 7 个宏，分别切换为我的 Mac 中可能会用到的浏览器。
...
这 7 个宏的动作基本一致，都只有两个步骤，第一步执行 Shell 脚本切换默认浏览器，第二步执行 AppleScript 进行确认。需要注意的是，在 Keyboard Maestro 中执行 Shell 脚本，需要指定可执行文件的绝对路径，否则会报错。由于我在 M1 Mac 上使用 Homebrew 安装 defaultbrowser，因此第一栏中写的是：

```shell
/opt/homebrew/bin/defaultbrowser browser
```

这 7 个宏设置的触发条件都是同一个快捷键 `⇧ + ⌥ + Right Arrow`，目的是为了利用 Keyboard Maestro 的 [Conflict Palette][28]，而这正是能实现快捷切换的关键。
...
{{< imgcap title="通过 Keyboard Maestro 的 Conflict Palette 切换默认浏览器" src="https://p15.p3.n0.cdn.getcloudapp.com/items/xQuONYyA/9cf8cb2a-c83d-4f1a-8e4a-ecd4fef293a3.gif" >}}

在上面的演示图中，切换前的默认浏览器是 Safari，我一共按了两次键盘（有些按键没有被录制下来），将默认浏览器切换成了 Chrome。首先按下快捷键 `⇧ + ⌥ + Right Arrow`，由于这 7 个宏的触发方式都是这个快捷键，因此 Keyboard Maestro 会调出 Conflict Palette，将 7 个宏同时显示出来。注意到每个宏名称的首字母颜色是不同的，而这就是它们的快捷键，直接在键盘上按下对应的字母，比如 `c`，就可以将默认浏览器切换为 Chrome [^1]，而完全不需要使用鼠标进行点击，不得不说，Keyboard Maestro 不愧为名副其实的「键盘大师」。
...
{{< imgcap title="在 Chrome 中打开 Safari 中的当前标签页" src="https://p15.p3.n0.cdn.getcloudapp.com/items/yAuAnZdg/33c406fa-47d3-4c2f-b4ac-1f3ad78dc8d3.gif" >}}
...
尽管如此，单独下载一个 App 并非必选项，挖掘自动化应用的潜力，可以实现同样的功能，足以省下一些不必要的开支。例如 [BetterTouchTool][31] 就可以实现 [隐藏菜单栏图标][32] 的功能，而使用 [Alfred][33]，则可以通过键盘轻松 [新建文件][34]。

Keyboard Maestro 是 macOS 上一个非常强大的自动化应用，如果善加利用，足以代替很多不必要的 App。本文介绍了使用 Keyboard Maestro 在 Mac 上实现切换输入法和浏览器，这是两个很常见的需求，可以极大地提高使用 Mac 的使用效率，希望对你有所帮助。
